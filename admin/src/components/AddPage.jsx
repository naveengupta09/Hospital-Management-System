...
// HELPER FUNCTIONS
//this function will give output in minutes and accoording to that it will just manage AM : PM
...
// this function will convert (YYYY-MM-DD) to (DD MMM YYYY) format
...
const AddPage = () => {
  ...
  function handleImage(e) {
    ...
    if (form.imagePreview && form.imageFile) {
      try {
        ...
        console.warn("Failed to revoke previous image preview", err);
      }
    }
    setForm((p) => ({
      ...p,
  ...
  function removeImage() {
    if (form.imagePreview && form.imageFile) {
      try {
        ...
        console.warn("Failed to revoke image preview", err);
      }
    }
    ...
    try {
      ...
      if (form.imagePreview && form.imageFile) {
        try {
          ...
          console.warn("Failed to clean up image preview", err);
        }
      }
      ...
      if (fileInputRef.current) {
        try {
          ...
          console.warn("Failed to reset file input after submit", err);
        }
      }
  ...
  return (
    <div className={s.pageContainer}>
      <div className={s.maxWidthContainerLg + " " + s.headerFlexContainer}>
