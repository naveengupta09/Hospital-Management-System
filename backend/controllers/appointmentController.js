...
// HELPER FUNCTION
// his function will return a finite number.
const safeNumber = (v) => {
  const n = Number(v);
...
export async function getAppointments(req, res) {
  try {
    ...
    const items = await Appointment.find(filter)
      ...
      .skip(skip)
      .limit(limit)
      .populate("doctorId", "name speciality imageUrl")
      .lean();
  ...
  try {
    ...
    if (!resolvedCreatedBy && !req.query.mobile) {
      return res.status(400).json({
        success: false,
        message: "Missing required parameter: createBy or mobile",
      });
    }
  ...
  try {
    ...
    if (!clerkUserId)
      return res.status(401).json({
        success: false,
        message: "Unauthorized: Unable to resolve user",
      });
    ...
    if (!doctorId || !patientName || !mobile || !date || !time) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: doctorId, patientName, mobile, date, time",
      });
    }
...
export const getStats = async (req, res) => {
  try {
    ...
    const savenDayAgo = new Date();
    savenDayAgo.setDate(savenDayAgo.getDate() - 7);
    const recent = await Appointment.countDocuments({
      createdAt: { $gte: savenDayAgo },
    });
...
export const getAppointmentsByDoctor = async (req, res) => {
  try {
    ...
    const items = await Appointment.find(filter).sort({ date: 1, time: 1}).skip(skip).limit(limit).populate("doctorId", "name specialization imageUrl image").lean();
...
export async function getRegisteredUserCount(req, res) {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Server error while fetching registered user count",
    });
  }
}
