...
function parseScheduleInput(s) {
    ...
    try {
        ...
        console.error("Error parsing schedule input:", error);
        return {};
    }
    }
...
export async function createDoctorProfile(req, res) {
    ...
    try {
        ...
        return res.status(201).json({ success: true, message: "Doctor profile created successfully.", data: out, token });
    } catch (error) {
...
const normalized = docs.map((d) => ({ _id: d._id,
    id: d._id,
    name: d.name || "",
...
// to get doctor profile by id ie to fetch one doctor profile
...
// to toggle Availability of doctor profile
